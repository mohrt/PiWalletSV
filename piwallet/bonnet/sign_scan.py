...
Three-screen sequence wired up by :func:`run_sign_flow`:
...
2. :class:`ConfirmProposalScreen` — decodes the assembled blob,
   pre-runs :func:`verify_proposal` so a malformed proposal fails
   before the operator presses **A**, and renders a summary
   (in / out / fee / network).
3. :class:`PairingMultipartQrScreen` — reused from the companion
...
when the operator presses **B**. The worker polls that flag between
...
class ConfirmProposalScreen:
    ...
    def on_event(self, event: Event) -> None:
        ...
        if b == Button.B and k == EventKind.PRESS:
            ...
            self.result = "cancel"
            return
        # Only accept A when verification passed; pressing A on a
        # rejected proposal must not advance to the signer.
        if (
            b == Button.A
    ...
    def start_worker(state: _ScanState) -> None:
        def run() -> None:
            def on_progress(have: int, msg: str) -> None:
                # ``msg`` includes the "fragment X/Y" string when the
                # assembler has at least one fragment. We pull totals
                # from it where possible so the UI status line shows
                # a precise count, and fall back to the raw message
                # otherwise.
                with state.lock:
...
def run_sign_flow(
    ...
    *,
    ...
    Returns ``"exit"`` when the operator long-presses B at any point;
    ...
    except (vfy.ProposalVerificationError, sgn.SigningError) as exc:
    ...
    # ---- 6. Animate signed_tx envelope back to companion ---------------
