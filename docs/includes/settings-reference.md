...
Press **B** again to return to the wallet list without saving unsaved
changes on value rows.
...
Press **A** on an action row to open that sub-screen. Value-row drafts
are saved before the sub-screen opens.
...
| **System reset** | Factory wipe for resale or hand-off — see below. |

### System reset

**Settings → System reset** erases all signer state on the device:
...
then shows *Device reset* and returns to first-setup (disclaimer → new
