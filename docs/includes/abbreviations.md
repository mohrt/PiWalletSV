<!--
  Project-wide abbreviation glossary, auto-appended to every page by
  pymdownx.snippets (see `mkdocs.yml`). Each `*[ABBR]: definition`
  line is a markdown abbreviation declaration consumed by the `abbr`
  extension; it produces no visible output here, but every occurrence
  of `ABBR` anywhere in the docs is wrapped in `<abbr title="...">`
  so desktop hover and screen readers expose the long form.

  Add new entries alphabetically. Keep definitions short — one line
  is enough for a tooltip. If a term needs more than a tooltip, link
  to the relevant doc page from the file that uses it.
-->
